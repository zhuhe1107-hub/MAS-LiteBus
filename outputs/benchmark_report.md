...
_(每模式重复运行 5 次, 易抖动指标显示 mean ± std)_
...
| 通信字符数 (text_chars 或 protocol_chars) | 51145 | 21440 | 22320 | 15379 | 16950 | 17034 |
| 估算 token (chars / 1.8) | 28414 | 11911 | 12400 | 8544 | 9417 | 9463 |
| 非文本状态传递次数 | 0 | 0 | 0 | 40 | 40 | 40 |
| 状态字节数 (累计) | 0 | 0 | 0 | 25600 | 25600 | 25600 |
| 总耗时 (秒) | 0.327754 ± 0.028014 | 0.326475 ± 0.021575 | 0.486560 ± 0.018340 | 0.185834 ± 0.024883 | 0.465191 ± 0.010793 | 0.532493 ± 0.034644 |
...
| IPC 发送帧数 | - | - | - | - | - | 44 |
| IPC 接收帧数 | - | - | - | - | - | 44 |
| IPC 出向字节数 (含 4B 长度头) | - | - | - | - | - | 25155 ± 39.9 |
| IPC 单次往返平均 (μs) | - | - | - | - | - | 7131.5 ± 532.3 |
| shm 分配次数 (POSIX) | - | - | - | - | - | 30 |
| shm 峰值字节 | - | - | - | - | - | 1536 |
...
- **text (累积)** 相对 text_v2 (合理基线): token -138.6%, 通信字符 -138.5%, 耗时 -0.4%, 本地检索次数 +0.0%, 消息数 +0.0%, 命中率 +0.0%
- **text + memory** 相对 text_v2 (合理基线): token -4.1%, 通信字符 -4.1%, 耗时 -49.0%, 本地检索次数 +70.0%, 消息数 +0.0%, 命中率 +80.0%
- **protocol (no memory)** 相对 text_v2 (合理基线): token +28.3%, 通信字符 +28.3%, 耗时 +43.1%, 本地检索次数 +0.0%, 消息数 -10.0%, 命中率 +0.0%
- **protocol (full)** 相对 text_v2 (合理基线): token +20.9%, 通信字符 +20.9%, 耗时 -42.5%, 本地检索次数 +70.0%, 消息数 -10.0%, 命中率 +80.0%
- **protocol + IPC** 相对 text_v2 (合理基线): token +20.6%, 通信字符 +20.6%, 耗时 -63.1%, 本地检索次数 +70.0%, 消息数 -10.0%, 命中率 +80.0%
...
- 共 44 次 send + 44 次 recv, 单次往返均值 7131.5 μs.
