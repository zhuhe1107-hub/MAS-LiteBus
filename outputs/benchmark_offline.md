...
| 通信字符数 (text_chars 或 protocol_chars) | 51145 | 21440 | 22299 | 15379 | 16950 |
| 估算 token (chars / 1.8) | 28414 | 11911 | 12388 ± 17.0 | 8544 | 9417 |
| 非文本状态传递次数 | 0 | 0 | 0 | 40 | 40 |
| 状态字节数 (累计) | 0 | 0 | 0 | 25600 | 25600 |
| 总耗时 (秒) | 0.356124 ± 0.016960 | 0.347141 ± 0.024166 | 0.547338 ± 0.014876 | 0.182896 ± 0.009629 | 0.513414 ± 0.010346 |
| 记忆检索次数 | 0 | 0 | 10 | 0 | 10 |
...
- **text (累积)** 相对 text_v2 (合理基线): token -138.6%, 通信字符 -138.5%, 耗时 -2.6%, 本地检索次数 +0.0%, 消息数 +0.0%, 命中率 +0.0%
- **text + memory** 相对 text_v2 (合理基线): token -4.0%, 通信字符 -4.0%, 耗时 -57.7%, 本地检索次数 +70.0%, 消息数 +0.0%, 命中率 +80.0%
- **protocol (no memory)** 相对 text_v2 (合理基线): token +28.3%, 通信字符 +28.3%, 耗时 +47.3%, 本地检索次数 +0.0%, 消息数 -10.0%, 命中率 +0.0%
- **protocol (full)** 相对 text_v2 (合理基线): token +20.9%, 通信字符 +20.9%, 耗时 -47.9%, 本地检索次数 +70.0%, 消息数 -10.0%, 命中率 +80.0%
...
|---|---:|---:|---:|---:|
| +memory 单独贡献 (NL 不变) | -4.0% | -4.0% | +70.0% | 80.0% |
| +protocol 单独贡献 (memory 不变) | +28.3% | +28.3% | +0.0% | 0.0% |
| +protocol + memory 合计 | +20.9% | +20.9% | +70.0% | 80.0% |

两项单独收益叠加 +24.3% vs 合计 +20.9% — 差值反映两个机制并非线性可加, memory 命中后 retrieve 文本变短, 协议封装受益于此, 因此合计大于纯线性叠加.
